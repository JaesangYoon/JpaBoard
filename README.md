...
2024.04 ~ 2024.05
## URL 
[http://43.203.239.135:8080](http://43.203.239.135:8080/)
## ERD 
...
- **Spring Data JPA**: JpaRepository 인터페이스를 통해 Spring Data JPA를 사용하여 데이터베이스와 상호작용했습니다. 이를 통해 기본적인 CRUD 연산을 간편하게 구현할 수 있었고, 복잡한 쿼리도 메서드 네이밍을 통해 자동으로 생성할 수 있었습니다. 또한, 데이터 접근 코드를 간결하게 유지하면서도, 높은 확장성을 확보할 수 있었습니다.
...
- 각각의 댓글에는 답글을 달 수 있으며, 부모 댓글이 삭제되면 해당 댓글의 답글도 함께 삭제됩니다. 
	- 이를 위해 Post(게시글) 엔티티의 `comments` 속성에 `orphanRemoval` 옵션을 `true`로 설정했습니다.
